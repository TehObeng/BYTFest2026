...
// --- STYLES ---
const pageTitleStyle = "text-left text-3xl sm:text-4xl md:text-5xl font-extrabold text-event-text-heading mt-0 mb-8 pb-4 border-b-2 border-gray-200";
const sectionTitleStyle = "text-left text-2xl sm:text-3xl font-bold text-event-blue mt-12 mb-8";
const subSectionTitleStyle = "text-left text-xl sm:text-2xl font-semibold text-event-text-heading mt-10 mb-6";
const paragraphStyle = "mb-6 leading-relaxed text-event-text-muted text-justify text-sm sm:text-base md:text-lg";
...
const tierDisplayConfig: Record<string, { titleHtml: string; customGridClasses?: string; tierTitleStyle: string; introBlurb?: string }> = {
  ...
  "Community Partner": { 
    titleHtml: "🤝 <span class='text-green-500'>Mitra Komunitas</span> Kami", 
  ...
  "Media Partner": { 
    titleHtml: "📰 <span class='text-sky-500'>Mitra Media</span> Kami", 
    tierTitleStyle: "text-md sm:text-lg font-semibold text-event-text-heading mt-10 mb-5 text-center",
...
interface SponsorshipPackageHighlight {
  ...
  keyBenefits: string[];
  idealFor?: string;
  colorClass: string; // Tailwind classes for border and background
  textColorClass?: string; // Tailwind class for prominent text if needed
  layoutClass?: string;
  isPremium?: boolean; 
}
...
const sponsorshipPackages: SponsorshipPackageHighlight[] = [
  { 
    ...
    keyBenefits: [
      ...
      "Eksposur brand maksimal di semua lini promosi (offline & online)",
    ...
    layoutClass: "lg:col-span-3 md:col-span-2", // Full width on md, spans 3 on lg
    isPremium: true,
  },
  { 
    ...
    colorClass: "border-sky-400 bg-sky-50 hover:border-sky-500",
...
const SponsorsPage: React.FC = () => {
  ...
  const closeModal = () => {
    ...
    // Delay clearing sponsor to allow modal exit animation
  ...
  return (
    ...
    >
      ...
      <motion.section variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mt-4">
            {whySponsorBenefits.map((benefit, index) => (
                <motion.div 
                    key={index} 
                    className="flex flex-col items-center text-center gap-3 sm:gap-4 p-6 bg-white rounded-2xl shadow-card hover:shadow-card-hover transition-all duration-300 ease-custom-ease transform hover:-translate-y-1"
                    custom={index}
                    variants={cardVariants}
                >
                    <span className="text-5xl sm:text-6xl text-event-blue">{benefit.icon}</span>
                    <div>
                        <h4 className="font-semibold text-event-blue-dark text-lg sm:text-xl mb-2">{benefit.title}</h4>
      ...
      <motion.section variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-6"> 
          {sponsorshipPackages.map((pkg, index) => (
            <motion.div 
              key={pkg.title} 
              className={`flex flex-col p-6 rounded-2xl border-2 shadow-lg hover:shadow-xl transition-all duration-300 ease-custom-ease transform hover:-translate-y-1.5 group ${pkg.colorClass} ${pkg.layoutClass || ''} ${pkg.isPremium ? 'border-l-[10px] border-t-4' : 'border-2'}`}
              custom={index}
              variants={cardVariants}
            >
              ...
              <button 
                ...
                className={`mt-auto w-full font-semibold py-3 px-4 rounded-lg text-sm sm:text-base transition-colors duration-200 shadow-button inline-flex items-center justify-center gap-2 ${pkg.isPremium ? 'bg-yellow-500 hover:bg-yellow-600 text-white' : 'bg-event-blue hover:bg-event-blue-dark text-white'}`}
              >
              ...
              if (sponsorsInTier && sponsorsInTier.length > 0) {
                return (
                  <motion.div 
                    ...
                    variants={sectionVariants} // Each tier section animates in
                  >
                    {config.introBlurb && (
                      <motion.p 
      ...
      <motion.section variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 text-center mt-6">
          {[
            ...
            <motion.div
              key={step.number}
              className="bg-gray-50 p-6 sm:p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
              custom={index}
              variants={cardVariants} 
            >
              <div className="text-6xl sm:text-7xl mb-5 text-event-blue">{step.icon}</div>
              <h4 className="text-xl sm:text-2xl font-semibold text-event-blue-dark mb-3">{step.number}. {step.title}</h4>
      ...
      <motion.section 
        ...
        className="text-center bg-gradient-to-br from-event-blue to-sky-600 hover:from-event-blue-dark hover:to-sky-700 transition-all text-white p-8 sm:p-10 md:p-12 rounded-2xl mt-12 shadow-xl" 
        variants={sectionVariants} 
      >
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-6">
            <a 
              ...
              className="inline-flex items-center justify-center gap-2.5 bg-white text-event-blue font-bold px-8 py-3.5 rounded-lg shadow-button hover:bg-gray-100 transition-colors duration-200 text-base sm:text-lg transform hover:scale-105"
            >
            ...
            <a 
              ...
              className="inline-flex items-center justify-center gap-2.5 border-2 border-white text-white font-bold px-8 py-3.5 rounded-lg hover:bg-white hover:text-event-blue transition-colors duration-200 text-base sm:text-lg transform hover:scale-105"
            >
